...
- **I can't always tell "touching" from "just standing nearby."** Since the model can't specifically detect hands, I approximate by using the person's entire body position instead. This mostly works, but can cause false positives — for example, an object sitting near someone's hip or leg can get flagged as "touched" even if their hands are nowhere near it. I tried a different technique (looking for the part of the person that was moving the most, frame to frame) to narrow this down without adding a new tool, but testing showed it wasn't reliable enough to trust — it kept picking up unrelated movement like clothing shifting. A proper fix would need real hand-tracking; due to time constraints I did not implement it.

- **Depth is invisible to a flat video.** Even a well-designed detection system can't tell if an object is truly touching someone or just happens to overlap them from the camera's angle, because a 2D video has no sense of "close to the camera" vs "far away." I actually proved this with real numbers: one incorrect "touch" result had a higher overlap percentage (86.5%) than a real, confirmed touch (16.4%) — so overlap amount alone can't be used to fix this. It's a fundamental limit of working with a single flat camera angle instead of a 3D-aware system.
